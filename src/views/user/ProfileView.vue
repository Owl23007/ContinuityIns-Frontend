<template>
  ...
  >
    ...
    <section v-if="user" class="articles-section">
      ...
      <profile-articles
        :user-id="route.params.id || user.id"
        :is-own-profile="isOwnProfile"
      />
    </section>
    ...
    <Teleport to="body">
      ...
      <!-- 头像上传模态框 -->
      <modal-dialog
        v-if="activeModal === 'avatar' && authStore.token"
        title="更换头像"
        @close="closeModal"
      >
        <avatar-uploader
      ...
      <!-- 背景上传模态框 -->
      <modal-dialog
        v-if="activeModal === 'background' && authStore.token"
        title="更换背景"
        @close="closeModal"
      >
        <background-uploader
