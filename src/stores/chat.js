...
export const useChatStore = defineStore("chat", () => {
  ...
  const createNewSession = (modelId = "deepseek-v3") => {
    ...
    const currentSession = chatSessions.value.find(
  ...
  const deleteSession = (sessionId) => {
    ...
    if (currentSessionId.value === sessionId) {
      if (chatSessions.value.length > 1) {
        // 如果有其他会话，选择下一个或上一个
        const nextIndex =
          index === chatSessions.value.length - 1 ? index - 1 : index + 1;
  ...
  const addMessage = (message) => {
    ...
    if (
      ...
      currentSession.value.messages.length <= 2
    ) {
      // 取前20个字符
  ...
  const updateMessage = (index, messageUpdate) => {
    if (!currentSession.value || index >= currentSession.value.messages.length)
      return;
