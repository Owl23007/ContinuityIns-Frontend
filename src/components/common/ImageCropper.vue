<template>
  <div class="image-cropper" ref="cropperWrapper">
    <div class="cropper-container" :style="containerStyle">
      <img v-if="imageUrl" :src="imageUrl" ref="cropperImage" class="crop-image" @load="onImageLoaded" />
  ...
  setTimeout(() => {
    cropper.value = new Cropper(cropperImage.value, {
      ...
      ready: () => {
        ...
        const cropperElements = document.querySelectorAll('.cropper-container, .cropper-drag-box, .cropper-face, .cropper-view-box, .cropper-point, .cropper-line');
...
.btn-rotate, .btn-zoom-in, .btn-zoom-out, .btn-reset {
...
.btn-rotate:hover, .btn-zoom-in:hover, .btn-zoom-out:hover, .btn-reset:hover {
